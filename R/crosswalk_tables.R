...
#' @param tty_rank (Optional). Vector of tty values for the given sab in the order of rank. If
...
write_crosswalk_table <-
  function(sab,
    ...
    df <-
            ...
            tibble::rowid_to_column("fct_vector_index")


    bottom_rank <-
            max(df$fct_vector_index)+1
    ...
    sql_statement <-
      c(
        ...
        "  CASE  ",
        mapply(sprintf,
          "    WHEN %s = '%s' AND %s = '%s' AND %s = '%s' THEN %s",
          "tty",
          df$fct_vector,
          "ispref",
          df$preferred,
